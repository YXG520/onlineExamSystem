...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath }/css/tp_trainingDd.css" />
	...
	<div id="practicing">
			...
			<div class="answerSheet">
				...
				<div class="dtk">
					...
					<div class="part" style="height: 200px;">
						<p>多选题第({{info.length}}道) 错 {{falseNumber}},对 {{trueNumber}}</p>
						<div class="que_num">
							<ul>
								<li @click="datiArea" v-for="(item,index) in info"
		...
		<div class="que_block floatright">
			<div class="que_block_up" :style="'font-size:'+ fs+'px'">
				当前作答第{{parseInt(index)+1}}道判断题目，科目代码：{{info[0].subjectCode}} <br>
				{{info[index].topic}}<br> <span v-show="ifShow"><span
					style="color: blue;" v-if="whetherRight=='回答正确' ">回答正确</span><span
					style="color: red;" v-if="whetherRight=='回答错误' ">回答错误,正确答案是
						{{ info[index].result}}</span></span>
			</div>
			<div class="part_select">
				<!-- 多选框 -->
				<div class="parting_sel">
					<div ref="ddResult" class="selabel">
						<input type="radio" name="judgeA" value="A" class="ddRadio" /><span>A.{{optionList[0]}}</span><br>
						<input type="radio" name="judgeB" value="B" class="ddRadio" /><span>B.{{optionList[1]}}</span><br>
						<input type="radio" name="judgeC" value="C" class="ddRadio" /><span>C.{{optionList[2]}}</span><br>
						<input type="radio" name="judgeD" value="D" class="ddRadio" /><span>D.{{optionList[3]}}</span>
					</div>
					<div class="nextPrev">
						<!-- 隐藏框，方便纯js文件引用jstl的值 -->
						<input id="rootUrl" type="hidden"
							...
							type="button" class="next" @click="btnClick" value="下一题" /> <input
							type="button" class="confirm" @click="verifyAnswer" value="确认答案" />
...
<script src="${pageContext.request.contextPath }/js/commonUtils.js"
	type="text/javascript" charset="utf-8"></script>
<script src="${pageContext.request.contextPath }/js/tp_trainingDd.js"
	type="text/javascript" charset="utf-8"></script>
